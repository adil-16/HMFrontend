...
const RoomCard = ({ room }) => {
  return (
    <div className="bg-black border border-white p-3 text-orange rounded-lg w-auto sm:w-80 font-Nunitoo">
      ...
      <h3 className="text-lg font-semibold mb-6 text-center">Room Number: {room.roomNumber} <span className="text-white">| ({room.roomType})</span></h3>
      
      {/* Beds */}
      <div>
        {room.beds?.map((bed, index) => (
          <div key={index} className="flex justify-between items-center mb-2">
            <p>Bed No: <span className="text-white">{bed.bedNumber} |</span>  Rate: <span className="text-white">({bed.bedRate})</span></p>
            {!bed.isBooked ?
